...
!        NOTES: The default should be sw(1:25)=1.0; Uses the full Ap history
!        if sw(9) = -1.0; sw(9)=-1 uses the full ap history and is supposed
!         to give the best results for geomagentic storms; sw(9) = 0 turns off
!        all ap variations.
!
...
SUBROUTINE GTD7(IYD,SEC,ALT,GLAT,GLONG,STL,F107A,F107,AP,MASS,D,T)
    ...
    IF(V1 == 1. .OR. ALT > ZN2(1) .OR. ALAST > ZN2(1) .OR. MSS /= MSSL) THEN
    ...
    IF(ALT >= ZN2(1)) THEN
        ...
        IF(MASS /= 0) then
            ...
            ! TOTAL MASS DENSITY
            IF(MASS == 48) THEN
                D(6) = 1.66E-24*( 4.*D(1) + 16.*D(2) + 28.*D(3) + 32.*D(4)&
                                  + 40.*D(5) + D(7)+ 14.*D(8) )
...
SUBROUTINE GHP7(IYD,SEC,ALT,GLAT,GLONG,STL,F107A,F107,AP,D,T,PRESS)
    ...
    do
        ...
        ! break out of loop
        IF(ABS(DIFF) < TEST .OR. L == LTEST) exit
    ...
    IF(mess.and.(L == LTEST)) WRITE(konsol,100) PRESS,DIFF
...
FUNCTION VTST7(IYD,SEC,GLAT,GLONG,STL,F107A,F107,AP,IC)
    ...
    IF( IYD /= IYDL(IC) .or. SEC /= SECL(IC) .or.&
...
SUBROUTINE GTS7(IYD,SEC,ALT,GLAT,GLONG,STL,F107A,F107,AP,MASS,D,T)
    ...
    COMMON/GTS3C/TLB,S,DB04,DB16,DB28,DB32,DB40,DB48,DB01,ZA,T0,Z0&
        ,G0,RL,DD,DB14,TR12
    COMMON/MESO7/TN1(5),TN2(4),TN3(5),TGN1(2),TGN2(2),TGN3(2)
    DIMENSION D(9),T(2),MT(10),AP(7),ALTL(8)
    COMMON/LOWER7/PTM(10),PDM(10,8)
    ...
    DATA MT/48,4,16,28,32,40,1,49,14,17/
    ...
    IF(ALT > ZN1(1)) THEN
        IF( V2 == 1. .OR. ALAST <= ZN1(1) ) then
            TINF = GLOBE7(YRD,SEC,GLAT,GLONG,STL,F107A,F107,AP,PT) 
            TINF = PTM(1)*PT(1)*( 1. + SW(16)*TINF )
        end if
    ELSE
        TINF = PTM(1)*PT(1)
    ENDIF

    IF(TNMOD > 600) TINF = TNMOD !.. PGR
    T(1) = TINF

    ! GRADIENT VARIATIONS NOT IMPORTANT BELOW ZN1(5)
    IF(ALT > ZN1(5)) THEN
        IF( V2 == 1 .OR. ALAST <= ZN1(5) ) then
            G0 = GLOBE7(YRD,SEC,GLAT,GLONG,STL,F107A,F107,AP,PS)
            G0 = PTM(4)*PS(1)*( 1. + SW(19)*G0 )
        end if
    ELSE
        G0 = PTM(4)*PS(1)
    ENDIF
    ...
    IF(V2 == 1. .OR. ALT < 300.) then
        TLB = GLOBE7(YRD,SEC,GLAT,GLONG,STL,F107A,F107,AP,PDA4) 
        TLB = PTM(2)*( 1. + SW(17)*TLB )*PDA4(1)
    end if
    ...
    IF (MASS == 0) then 
        ...
        G28 = SW(21)*GLOBE7(YRD,SEC,GLAT,GLONG,STL,F107A,F107,AP,PDA3)
        DAY = AMOD(YRD, 1000.)
    
        ! VARIATION OF TURBOPAUSE HEIGHT
        ZHF = PDL(25,2)*( 1. + SW(5)*PDL(25,1)*SIN( DGTR*GLAT )*COS( DR*DAY - DR*PT(14) ) )
        YRD = IYD
        T(1) = TINF
        XMM = PDM(5,3)
        Z   = ALT

        ! If mass is invalid, warn and don't do anything
        if ( MASS /= 1  .and.&
             MASS /= 4  .and.&
             MASS /= 14 .and.&
             MASS /= 16 .and.&
            ...
            if ( mass == 48 .or. mass == 4) then
                ...
                ! Diffusive density at Alt
                DD = DENSU(Z,DB04,TINF,TLB, 4.,ALPHA(1),T(2),PTM(6),S,MN1,ZN1,TN1,TGN1)
                D(1) = DD
                IF( Z <= ALTL(1) .and. SW(15) /= 0. ) then
                    ! Turbopause
                    ZH04 = PDM(3,1)
                    ! Mixed density at Zlb
            ...
            if ( mass == 48 .or. mass == 16 .or. mass == 49 ) then
                ! **** O DENSITY ****
            ...
            if ( mass == 48 .or. mass == 32 .or. mass == 49 ) then
                ! **** O2 DENSITY ****
...
FUNCTION GLOBE7(YRD,SEC,LAT,LONG,TLOC,F107A,F107,AP,P)
    ...
    IF(TLL /= TLOC) then
        IF(SW(7) /= 0 .or. SW(8) /= 0 .or. SW(14) /= 0) then
            STLOC = SIN(HR*TLOC)
            CTLOC = COS(HR*TLOC)
    ...
    DF   = F107 - F107A
    DFA  = F107A - 150.
    T(1) =  P(20)*DF*( 1. + P(60)*DFA ) + P(21)*DF**2 &
            + P(22)*DFA + P(30)*DFA**2
    F1   = 1. + ( P(48)*DFA + P(20)*DF + P(21)*DF**2 )*SWC(1)
    F2   = 1. + ( P(50)*DFA + P(20)*DF + P(21)*DF**2 )*SWC(1)
    ...
    IF(SW(7) /= 0) then 
        ...
        T(7) = ( P(4)*PLG(2,2) + P(5)*PLG(4,2) + P(28)*PLG(6,2) + T71 )*CTLOC + &
    ...
    ! TERDIURNAL
    IF(SW(14) /= 0) then
        T(14) = ( P(40)*PLG(4,4) + ( P(94)*PLG(5,4) + P(47)*PLG(7,4) )*CD14*SWC(5) )*S3TLOC +&
    ...
    IF(SW9 /= -1.) then
        ...
        IF(SW(9) /= 0) then
            T(9) = P(33) + P(46)*PLG(3,1) + P(35)*PLG(5,1) + &
                   ( P(101)*PLG(2,1) + P(102)*PLG(4,1) + P(103)*PLG(6,1) )*CD14*SWC(5) + &
                   ( P(122)*PLG(2,2) + P(123)*PLG(4,2) + P(124)*PLG(6,2) )*SWC(7)*COS( HR*( TLOC - P(125) ) )
            T(9) = APDF*T(9)
        end if
    else
        IF(P(52) /= 0) then
            EXP1 = -10800.*ABS(P(52))/( 1. + P(139)*( 45. -ABS(LAT) ) )
            EXP1 = MIN( EXP(EXP1), .99999)
            ...
            IF(SW(9) /= 0) then
                T(9) = P(51) + P(97)*PLG(3,1) + P(55)*PLG(5,1) + &
                       ( P(126)*PLG(2,1) + P(127)*PLG(4,1) + P(128)*PLG(6,1) )*CD14*SWC(5) + &
                       ( P(129)*PLG(2,2) + P(130)*PLG(4,2) + P(131)*PLG(6,2) )*SWC(7)*COS( HR*( TLOC - P(132) ) )
                T(9) = APT(1)*T(9)
            end if
        end if
    end if

    IF(SW(10) /= 0 .and. LONG > -1000.) then

        ! LONGITUDINAL
        IF(SW(11) /= 0) then
            tmp1 = P(110)*PLG(2,2) + P(111)*PLG(4,2) + P(112)*PLG(6,2)
            tmp1 = SWC(5)*tmp1*CD14
            tmp1 = tmp1 + P(65)*PLG(3,2) + P(66)*PLG(5,2) + P(67)*PLG(7,2) &
                   + P(104)*PLG(2,2) + P(105)*PLG(4,2) + P(106)*PLG(6,2) 
            tmp2 = P(113)*PLG(2,2)+P(114)*PLG(4,2)+P(115)*PLG(6,2)
            tmp2 = SWC(5)*tmp2*CD14
            tmp2 = tmp2 + P(91)*PLG(3,2) + P(92)*PLG(5,2) + P(93)*PLG(7,2) &
                   + P(107)*PLG(2,2) + P(108)*PLG(4,2) + P(109)*PLG(6,2) 
            T(11) = ( 1. + P(81)*DFA*SWC(1) )*( tmp1*COS(DGTR*LONG) + tmp2*SIN(DGTR*LONG) )
        end if

        ! UT AND MIXED UT,LONGITUDE
        IF(SW(12) /= 0) then
            tmp1 = ( P(69)*PLG(2,1) + P(70)*PLG(4,1) + P(71)*PLG(6,1) ) &
                   * COS( SR*( SEC - P(72) ) )&
                   * ( 1. + P(96)*PLG(2,1) ) &
                   * ( 1. + P(82)*DFA*SWC(1) ) &
                   * ( 1. + P(120)*PLG(2,1)*SWC(5)*CD14 )
            tmp2 = ( P(77)*PLG(4,3) + P(78)*PLG(6,3) + P(79)*PLG(8,3) )&
                   * COS( SR*( SEC - P(80) ) + 2.*DGTR*LONG ) &
                   * SWC(11)*( 1. + P(138)*DFA*SWC(1) )
            T(12) = tmp1 + tmp2
        end if
        ...
        IF(SW(13) /= 0) then
            IF(SW9 /= -1.) then
                tmp1 = P( 61)*PLG(3,2) + P( 62)*PLG(5,2) + P( 63)*PLG(7,2)
                tmp2 = P(116)*PLG(2,2) + P(117)*PLG(4,2) + P(118)*PLG(6,2)
                tmp3 = P( 84)*PLG(2,1) + P( 85)*PLG(4,1) + P( 86)*PLG(6,1)

                T(13) = SWC(11)*tmp1* COS( DGTR*( LONG - P(64) ) ) *( 1. + P(121)*PLG(2,1) ) &
                      + SWC(11)*tmp2* COS( DGTR*( LONG - P(119)) ) *SWC(5)*CD14 & 
                      + SWC(12)*tmp3* COS(   SR*( SEC  - P(76) ) )
                T(13) = APDF*T(13)
            else
                IF(P(52) /= 0) then
                    tmp1 = P( 53)*PLG(3,2) + P( 99)*PLG(5,2) + P( 68)*PLG(7,2)
                    tmp2 = P(134)*PLG(2,2) + P(135)*PLG(4,2) + P(136)*PLG(6,2)
                    tmp3 = P( 56)*PLG(2,1) + P( 57)*PLG(4,1) + P( 58)*PLG(6,1)

                    T(13) = SWC(11)*tmp1* COS( DGTR*( LONG - P( 98) ) ) *( 1. + P(133)*PLG(2,1) ) &
                          + SWC(11)*tmp2* COS( DGTR*( LONG - P(137) ) ) *SWC(5)*CD14 &
                          + SWC(12)*tmp3* COS(   SR*( SEC  - P( 59) ) )
                    T(13) = APT(1) * T(13)
                end if
            end if
        end if
    ...
    TINF = P(31)
    TINF = P(31) + SUM( ABS(SW(1:NSW))*T(1:NSW) )
...
FUNCTION GLOB7S(P)
    ...
    IF(SW(9) /= 0) then
        IF(SW(9) == 1) then
            ...
            T(9) = APT(1)*( P(51) + P(97)*PLG(3,1)*SWC(2) )
        end if
    end if

    IF(SW(10) /= 0 .and. SW(11) /= 0 .and. LONG > -1000.) then
        ! LONGITUDINAL
        tmp1 = P(81)*SWC(5)*COS(    DR*( DAY - P(82) ) ) &
             + P(86)*SWC(6)*COS( 2.*DR*( DAY - P(87) ) )

        tmp1 = 1. + tmp1*PLG(2,1) &
             + P(84)*SWC(3)*COS(    DR*( DAY - P(85) ) ) &
             + P(88)*SWC(4)*COS( 2.*DR*( DAY - P(89) ) ) 
        tmp2 = ( P(65)*PLG(3,2) + P(66)*PLG(5,2) + P(67)*PLG(7,2) &
               + P(75)*PLG(2,2) + P(76)*PLG(4,2) + P(77)*PLG(6,2) )*COS(DGTR*LONG) &
             + ( P(91)*PLG(3,2) + P(92)*PLG(5,2) + P(93)*PLG(7,2) &
               + P(78)*PLG(2,2) + P(79)*PLG(4,2) + P(80)*PLG(6,2) )*SIN(DGTR*LONG)
        T(11) = tmp1 * tmp2
    end if

    TT = SUM( ABS(SW(1:14))*T )
...
SUBROUTINE SPLINEM(X,Y,N,YP1,YPN,Y2)
    ...
    DO I=2,N-1
        ...
        U(I) = U(I) - ( Y(I) - Y(I-1) )/( X(I) - X(I-1) )
        U(I) = 6. * U(I)/( X(I+1) - X(I-1) ) - SIG*U(I-1) 
        U(I) = U(I)/P
    end do
...
SUBROUTINE SPLINTM(XA,YA,Y2A,N,X,Y)
    ...
    H = XA(KHI) - XA(KLO)
    IF( mess .and. H == 0) WRITE(konsol,*) 'BAD XA INPUT TO SPLINT'
    A = ( XA(KHI) - X )/H
    B = ( X - XA(KLO) )/H
    Y = ( A**3 - A )*Y2A(KLO) + (B**3 - B)*Y2A(KHI)
    Y = Y * H**2/6
    Y = Y + A*YA(KLO) + B*YA(KHI) 
    RETURN
END SUBROUTINE SPLINTM
...
SUBROUTINE SPLINI(XA,YA,Y2A,N,X,YI)
    ...
    DO WHILE (X > XA(KLO) .AND. KHI <= N)
        ...
        A = ( XA(KHI) - XX )/H
        B = ( XX - XA(KLO) )/H
        tmp = ( -(1. + A**4)/4. + A**2/2. )*Y2A(KLO)
        tmp = tmp + ( B**4/4. - B**2/2. )*Y2A(KHI)
        tmp = tmp * H**2/6
        tmp = tmp + ( 1. - A**2 )*YA(KLO)/2. 
        tmp = tmp + B**2*YA(KHI)/2.
        YI = YI + tmp*H
        KLO = KLO+1
        KHI = KHI+1
    END DO
...
FUNCTION DNET(DD, DM, ZHM, XMM, XM)
    ...
    ! If either density dominates, use it.
    if (YLOG < -10.) then
        DNET = DD
        return
    else if (YLOG > 10) then
        DNET = DM
        return
    endif
