        ...
        DO 6 K=1,3,2
            ...
            2         DO 3 M=3,I,2
                H(IL+M+1)=G(IL+M+1)+Z*H(IH+M+1)+X*(H(IH+M+3)-&
                    H(IH+M-1))-Y*(H(IH+M+2)+H(IH+M-2))
                3             H(IL+M)=G(IL+M)+Z*H(IH+M)+X*(H(IH+M+2)-&
                    H(IH+M-2))+Y*(H(IH+M+3)+H(IH+M-1))
...
SUBROUTINE FELDCOF(YEAR)
    ...
    DO 1234 J=1,3
        ...
        DO 9 N=1,NMAX
            ...
            DO 9 M=1,N
                F = F * (X + M) / (X - M + 1.D0)
                IF(IS == 0) F = F * DSQRT((X - M + 1.D0) / (X + M))
                GH1(I) = GHA(I-1) * F
                GH1(I+1) = GHA(I) * F
                I=I+2
                9     CONTINUE
...
real function OVL_ANG(sla,slo,cla,clo,rr)
    ...
    if(abs(sla) >= 89.99.or.abs(cla) >= 89.99.or.&
        abs(sla) < 30.) then
        OVL_ANG = 999.99
        return
    endif
...
do ihem = ih,3
    ...
    DO 7 I = 1,2
        ...
        15   BB3 = ABS(B3 - B2)
        BB2 = ABS(BM - B2)
        IF(BB2 < DHH.AND.BB3 < DHH) GO TO 21
        17    BM = BM
        XGEO = XBM
        YGEO = YBM
        ZGEO = ZBM
...
SUBROUTINE GEOCOR(SLA,SLO,RH,DLA,DLO,CLA,CLO,PMI)
    ...
    IF(R <= RH) GOTO 11
    DCL = C - 1.5707963268
    IF(ABS(DCL) <= HHH) GOTO 9
    RZM = ZM
    IF(SZM > 0..AND.RZM > 0.) GOTO 1
    IF(SZM < 0..AND.RZM < 0.) GOTO 1
    R = R1
    X = X1
    Y = Y1
...
SUBROUTINE IGRF(IY,NM,R,T,F,BR,BT,BF)
    ...
    DO 20 N=1,11
        ...
        DO 20 M=1,N
            ...
            IF (IYR  <  1910) GOTO 1905      !INTERPOLATE BETWEEN 1905 - 1910
            IF (IYR  <  1915) GOTO 1910      !INTERPOLATE BETWEEN 1910 - 1915
            IF (IYR  <  1920) GOTO 1915      !INTERPOLATE BETWEEN 1915 - 1920
            IF (IYR  <  1925) GOTO 1920      !INTERPOLATE BETWEEN 1920 - 1925
            IF (IYR  <  1930) GOTO 1925      !INTERPOLATE BETWEEN 1925 - 1930
            IF (IYR  <  1935) GOTO 1930      !INTERPOLATE BETWEEN 1930 - 1935
            IF (IYR  <  1940) GOTO 1935      !INTERPOLATE BETWEEN 1935 - 1940
            IF (IYR  <  1945) GOTO 1940      !INTERPOLATE BETWEEN 1940 - 1945
            IF (IYR  <  1950) GOTO 1945      !INTERPOLATE BETWEEN 1945 - 1950
            IF (IYR  <  1955) GOTO 1950      !INTERPOLATE BETWEEN 1950 - 1955
            IF (IYR  <  1960) GOTO 1955      !INTERPOLATE BETWEEN 1955 - 1960
            IF (IYR  <  1965) GOTO 1960      !INTERPOLATE BETWEEN 1960 - 1965
            IF (IYR  <  1970) GOTO 1965      !INTERPOLATE BETWEEN 1965 - 1970
            IF (IYR  <  1975) GOTO 1970      !INTERPOLATE BETWEEN 1970 - 1975
            IF (IYR  <  1980) GOTO 1975      !INTERPOLATE BETWEEN 1975 - 1980
            IF (IYR  <  1985) GOTO 1980      !INTERPOLATE BETWEEN 1980 - 1985
            IF (IYR  <  1990) GOTO 1985      !INTERPOLATE BETWEEN 1985 - 1990
            IF (IYR  <  1995) GOTO 1990      !INTERPOLATE BETWEEN 1990 - 1995
            IF (IYR  <  2000) GOTO 1995      !INTERPOLATE BETWEEN 1995 - 2000
            IF (IYR  <  2005) GOTO 2000      !INTERPOLATE BETWEEN 2000 - 2005
            IF (IYR  <  2010) GOTO 2005      !INTERPOLATE BETWEEN 2005 - 2010
            !
...
SUBROUTINE RECALC(IYR,IDAY,IHOUR,MIN,ISEC)
    ...
    REAL ST0,CT0,SL0,CL0,CTCL,STCL,CTSL,STSL,SFI,CFI,SPS,CPS,&
        ...
        EYMAGX,EYMAGY,GST,SLONG,SRASN,SDEC,BA(8),DECARG
